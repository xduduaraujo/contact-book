<template>
  <UBModal :isVisible="showModal" width="432px" height="342px">
    <UBTitle :text="title" class="modal-header" />
    ...
    <UBFooter width="100%" class="modal-footer">
      ...
      <UBButton :handleClick="handleSave" text="Salvar" :disabled="!formIsValid()" class="btn-save" />
    </UBFooter>
  </UBModal>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const formIsValid = () =>
      !!contactData.name || !!contactData.email || !!(contactData.telephone && contactData.telephone.length >= 14);
